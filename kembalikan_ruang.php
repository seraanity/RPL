...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "UPDATE data_pinjam_ruang SET status = '2', tgl_kembali = ? WHERE id_pinjam_ruang = ?";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "si", $tgl_kembali, $id_pinjam);

    if (mysqli_stmt_execute($stmt)) {
        // Pengembalian berhasil
        $_SESSION['message'] = "Ruang berhasil dikembalikan!";
    } else {
        // Pengembalian gagal
    ...
    header("Location: data_peminjaman_ruang.php");
    exit;
}
?>
